...
import ConversationSharePoster from '@/components/share/ConversationSharePoster';
import QuoteSharePoster from '@/components/share/QuoteSharePoster';
import {
  capturePosterImage,
  savePosterToAlbum,
  sharePosterImage,
...
export type SharePosterContent =
  | {
      kind: 'conversation';
  ...
  | { kind: 'quote'; message: ChatMessage };

interface ConversationShareModalProps {
  visible: boolean;
  content: SharePosterContent | null;
  onClose: () => void;
}

function getModalTitle(content: SharePosterContent): string {
  return content.kind === 'quote' ? '金句分享海报' : '对话分享海报';
}

export default function ConversationShareModal({
  visible,
  content,
  onClose,
      ...
      <View style={styles.overlay}>
        ...
        <View style={styles.sheet}>
          <View style={styles.sheetHeader}>
            <Text style={styles.sheetTitle}>{getModalTitle(content)}</Text>
            <TouchableOpacity
              onPress={handleClose}
              hitSlop={12}
              disabled={!!busy}
          ...
          >
            <View ref={posterRef} collapsable={false}>
              {content.kind === 'conversation' ? (
                <ConversationSharePoster
                  messages={content.messages}
                  channelMode={content.channelMode}
                  title={content.title}
                />
              ) : (
                <QuoteSharePoster message={content.message} />
              )}
            </View>
          </ScrollView>
